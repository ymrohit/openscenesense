...
def pick_first_available(candidates, client):
    # Probe models quickly with a tiny chat.completions call
    for m in candidates:
        try:
            ...
            print(f"[probe] Unavailable model '{m}': {e}")
    raise RuntimeError(f"None of the candidate models are available: {candidates}")
...
def main():
    ...
    # Import after env to avoid side effects
    import logging
    from openscenesense import OpenRouterAnalyzer, ModelConfig, AnalysisPrompts, DynamicFrameSelector

    # Free and capable OpenRouter models (ordered by expected capability/availability)
    # Vision candidates (VL): prefer Qwen VL then Llama 3.2 vision
    vision_candidates = [
        'qwen/qwen2.5-vl-7b-instruct:free',
        'qwen/qwen2.5-vl-32b-instruct:free',
        'meta-llama/llama-3.2-11b-vision-instruct:free',
    ]
    # Text candidates: prefer Qwen 72B then lighter fallbacks
    text_candidates = [
    ...
    # Create a lightweight OpenRouter client for probing
    from openai import OpenAI
    probe_client = OpenAI(api_key=OPENROUTER_API_KEY, base_url="https://openrouter.ai/api/v1")

    vision_model = pick_first_available(vision_candidates, probe_client)
    text_model = pick_first_available(text_candidates, probe_client)

    # Keep audio on stable Whisper for reliability in tests
    models = ModelConfig(
    ...
    prompts = AnalysisPrompts(
        frame_analysis=(
            "Analyze this frame focusing on visible elements, actions, and context."
        ),
        detailed_summary=(
            "Create a cohesive narrative combining visual and audio elements from this {duration:.1f}-second video.\n"
            "Timeline:\n{timeline}\nTranscript:\n{transcript}"
        ),
        brief_summary=(
